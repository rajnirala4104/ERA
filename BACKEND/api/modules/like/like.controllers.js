...
const likeControllersObject = {
   ...
   getAllLikesOfAPerticualarPost: expressAsyncHandler(async (req, res) => {
      try {
         const { postId } = req.params; //getting post id from req.params

         const allLikes = await Like.find(); // mongoose find query
         const post = await Post.find({ _id: postId }); //mongoose find query to itrate post with condition

         const likesOfAPerticualerPost = []; //empty array to store all likes of a perticular post

         for (let i = 0; i < allLikes.length; i++) {
            const singleLikeObject = allLikes[i]; //getting single like object in a variable
   ...
   createLike: expressAsyncHandler(async (req, res) => {
      try {
         ...
         const likeObject = await Like.find({
            user: logedInUserId,
            post: postId,
         });
