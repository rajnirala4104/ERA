...
import { FollowersPopupContext, ProfilePopupContext } from "../contaxt";
...
const UserProfileHeader: React.FC<user> = (props) => {
   ...
   const getAllTheFollowings = async () => {
      const followingResponse =
         await getAllTheFollowingsOfAPerticularUserApiCall(
            props._id!,
            loggedUser.token
         );
   ...
   let followersAndFollowingsReducFinalState: followeInterface[];
   let followersAndFollowingsReducSecondFinalState: followeInterface[];
   ...
   if (Array.isArray(followers) && Array.isArray(following)) {
      ...
      followersAndFollowingsReducFinalState = followers.map((singleObject) => ({
      ...
      followersAndFollowingsReducSecondFinalState = following.map(
         (singleObject) => ({
            user: singleObject.user,
            followedUserId: singleObject.followedUserId,
         })
   ...
   document.title = props.name ? props.name.toUpperCase() : "ERA";
